...
@section('content_header')
    <h1 class="m-0 text-dark fw-bolder">Servicios Generales</h1>
@stop
@section('css')
    <style>
        .statusService {
            ...
            <table class="table table-striped" id="service">
                ...
                <tbody>
                    @foreach ($services as $s )
                        ...
                        @include('admin.services.generales._edit')
                        @include('admin.services.generales._details')
                        @include('admin.services.generales._delete')
                    @endforeach
                </tbody>
            </table>
        </div>
...
@include('admin.services.generales._create')
@stop
@section('js')
<script>
                ...
                success: function(response){
                    if(response.success){
                        Swal.fire({
                            ...
                            window.location.href = "{{ route('services_index') }}";
                        });
                    }else{
                        Swal.fire({
                            type: 'error',
...
<script>
    ...
    $('#confirmDelete').on('click', function () {
        $.ajax({
            url: '/admin/services/' + deleteId + '/delete',
            type: 'DELETE',
            dataType: 'json',
            data: {
                _token: '{{ csrf_token() }}'
